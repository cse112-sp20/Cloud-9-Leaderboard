    ...
    context.subscriptions.push(vscode.languages.registerDocumentSymbolProvider(SEARCH_RESULT_SELECTOR, {
        provideDocumentSymbols(document, token) {
            const results = parseSearchResults(document, token)
                .filter(isFileLine)
                .map((line) => new vscode.DocumentSymbol(line.path, "", vscode.SymbolKind.File, line.allLocations
                .map(({ originSelectionRange }) => originSelectionRange)
                .reduce((p, c) => p.union(c), line.location.originSelectionRange), line.location.originSelectionRange));
            return results;
        },
        ...
        provideCompletionItems(document, position) {
            ...
            if (position.character === 0 ||
                ...
                return DIRECTIVES.filter((suggestion) => header.every((line) => line.indexOf(suggestion) === -1)).map((flag) => ({
                    label: flag,
            ...
            return FLAGS.filter((flag) => line.text.indexOf(flag) === -1).map((flag) => ({ label: flag, insertText: flag + " " }));
        },
        ...
        provideDefinition(document, position, token) {
            ...
            const translateRangeSidewaysBy = (r, n) => r.with({
                start: new vscode.Position(r.start.line, Math.max(0, n - r.start.character)),
                end: new vscode.Position(r.end.line, Math.max(0, n - r.end.character)),
            });
            return [
                Object.assign(Object.assign({}, lineResult.location), { targetSelectionRange: translateRangeSidewaysBy(lineResult.location.targetSelectionRange, position.character - 1) }),
            ];
        },
...
function parseSearchResults(document, token) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (resultLine) {
            ...
            const resultStart = (indentation +
                _lineNumber +
                seperator +
                resultIndentation).length;
            ...
            currentTargetLocations === null || currentTargetLocations === void 0 ? void 0 : currentTargetLocations.push(location);
            links[i] = {
                type: "result",
                location,
                isContext: seperator === " ",
