...
function activate(context) {
    ...
    context.subscriptions.push(vscode.languages.registerDocumentSymbolProvider(SEARCH_RESULT_SELECTOR, {
        ...
        provideDefinition(document, position, token) {
            ...
            const translateRangeSidewaysBy = (r, n) => r.with({ start: new vscode.Position(r.start.line, Math.max(0, n - r.start.character)), end: new vscode.Position(r.end.line, Math.max(0, n - r.end.character)) });
            return [Object.assign(Object.assign({}, lineResult.location), { targetSelectionRange: translateRangeSidewaysBy(lineResult.location.targetSelectionRange, position.character - 1) })];
        }
