    ...
    return __generator(this, function (_a) {
      switch (_a.label) {
        ...
        case 3:
          ...
          filtered_payloads = payloads.filter(function (n) {
            return n.day === day;
          });
...
function getCodeTimeSummary() {
  ...
  var filtered_payloads = payloads.filter(function (n) {
    return n.day === day;
  });
  if (filtered_payloads && filtered_payloads.length) {
...
function findTimeDataSummary(project) {
  ...
  if (payloads && payloads.length) {
    ...
    timeData = payloads.find(function (n) {
      return n.day === day && n.project.directory === project.directory;
    });
  }
  return timeData;
}
function saveTimeDataSummaryToDisk(data) {
  ...
  if (payloads && payloads.length) {
    ...
    var idx = payloads.findIndex(function (n) {
      return (
        n.day === data.day && n.project.directory === data.project.directory
      );
    });
    if (idx !== -1) {
      payloads[idx] = data;
