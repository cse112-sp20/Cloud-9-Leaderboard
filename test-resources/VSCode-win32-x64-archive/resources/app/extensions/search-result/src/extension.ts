    ...
    vscode.languages.registerDefinitionProvider(SEARCH_RESULT_SELECTOR, {
      provideDefinition(
        ...
        const translateRangeSidewaysBy = (r: vscode.Range, n: number) =>
          r.with({
            start: new vscode.Position(
              r.start.line,
              Math.max(0, n - r.start.character),
            ),
            end: new vscode.Position(
              r.end.line,
              Math.max(0, n - r.end.character),
            ),
          });
        ...
        return [
          {
            ...lineResult.location,
            targetSelectionRange: translateRangeSidewaysBy(
              lineResult.location.targetSelectionRange!,
    ...
    if (multiRootFormattedPath) {
      ...
      if (folder) {
        return vscode.Uri.file(
          pathUtils.join(folder.uri.fsPath, workspacePath),
        );
      }
    } else if (vscode.workspace.workspaceFolders.length === 1) {
      return vscode.Uri.file(
        pathUtils.join(vscode.workspace.workspaceFolders[0].uri.fsPath, path),
      );
